...
describe("reproducible core",()=>{
  ...
  it("builds a balanced four-trial quick session for every lab",()=>{const labs=["atlas","apprenticeship","state","anchor"] as const;for(const lab of labs){const protocol=buildProtocol(7,4,lab);expect(protocol).toHaveLength(4);expect(protocol.some(trial=>trial.condition==="sham"||trial.condition==="omission"||trial.condition==="preparation")).toBe(true);expect(protocol.some(trial=>trial.condition==="paired"||trial.condition==="repetition")).toBe(true)}});
